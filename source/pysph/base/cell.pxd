...
from pysph.base.carray cimport BaseArray, LongArray, DoubleArray, IntArray
...
cdef inline void construct_immediate_neighbor_list(IntPoint pnt, 
                                                   list neighbor_list, 
                                                   bint include_self=*)

cdef inline int find_hierarchy_level_for_radius(double radius, 
                                                double min_cell_size, 
                                                double max_cell_size,
                                                double cell_size_step, 
                                                int num_levels)
...
cdef class CellManager:
    ...
    cdef int get_potential_cells(self, Point pnt, double radius, 
                                 list cell_list, bint single_layer=*) except -1

    cdef int _get_cells_within_radius(self, Point pnt, double radius, 
                                      int level, list cell_list) except -1

    cdef int _get_one_layer_of_cells(self, Point pnt, 
